...
// Church configuration - would come from database/API based on church_id
...
// Default church config (Fielder example from screenshots)
const churchConfig: ChurchConfig = {
  name: "FIELDER",
  tagline: "Your Church Home",
  socialLinks: {
    facebook: "https://facebook.com/fielderchurch",
    youtube: "https://youtube.com/fielderchurch",
    twitter: "https://twitter.com/fielderchurch",
    tiktok: "https://tiktok.com/@fielderchurch",
    instagram: "https://instagram.com/fielderchurch"
  }
}
...
export default function LandingPage() {
  ...
  const { tier, isTrialActive, daysLeftInTrial, canAccessPremium } = useSubscription()
  const { t } = useLanguage()
  const [hasGenerated, setHasGenerated] = useState(false)

  // Auto-generate verse of the day on load if not already loaded
  useEffect(() => {
  ...
  // Render only the social icons that exist in church config
  const renderSocialIcons = () => {
    const icons = []
    const iconClass = "w-6 h-6 text-white hover:text-amber-400 transition-colors"
    
    if (churchConfig.socialLinks.facebook) {
      icons.push(
        <a key="fb" href={churchConfig.socialLinks.facebook} target="_blank" rel="noopener noreferrer">
          <Facebook className={iconClass} />
        </a>
      )
    }
    if (churchConfig.socialLinks.youtube) {
      icons.push(
        <a key="yt" href={churchConfig.socialLinks.youtube} target="_blank" rel="noopener noreferrer">
          <Youtube className={iconClass} />
        </a>
      )
    }
    if (churchConfig.socialLinks.twitter) {
      icons.push(
        <a key="tw" href={churchConfig.socialLinks.twitter} target="_blank" rel="noopener noreferrer">
          <XIcon className={iconClass} />
        </a>
      )
    }
    if (churchConfig.socialLinks.tiktok) {
      icons.push(
        <a key="tt" href={churchConfig.socialLinks.tiktok} target="_blank" rel="noopener noreferrer">
          <TikTokIcon className={iconClass} />
        </a>
      )
    }
    if (churchConfig.socialLinks.instagram) {
      icons.push(
        <a key="ig" href={churchConfig.socialLinks.instagram} target="_blank" rel="noopener noreferrer">
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col max-w-md mx-auto bg-[#0c1929] shadow-2xl">
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {/* ===== CHANGE 1: Church Branded Header (replaces original logo section) ===== */}
